...
// next/image isn't available in jsdom — stub it with a plain <img>
vi.mock('next/image', () => ({
...
describe('BookCard', () => {
  ...
  it('renders cover image when coverUrl is provided', () => {
    render(<BookCard book={baseBook} />);
    // alt="" is intentionally empty (decorative image — the title link is the accessible name).
    // Query by tag name since getByRole('img') excludes alt="" elements by design.
  ...
  it('renders placeholder when coverUrl is null', () => {
    ...
    // Placeholder renders the title as text inside the cover area
  ...
  it('omits year and editions when they are missing', () => {
    render(
      <BookCard book={{ ...baseBook, firstPublished: null, editionCount: 0 }} />,
    );
